...
typedef enum {
   ...
   opINC,     /* RR    reg(r) = reg(r) + 1, s and t ignored */
   opDEC,     /* RR    reg(r) = reg(r) - 1, s and t ignored */
   opSHL,     /* RR    reg(r) = reg(r) << reg(t) */
   opSHR,     /* RR    reg(r) = reg(r) >> reg(t) */
...
int opCodeOverallCounter[opLim + 1];
int opCodeSimulationCounter[opLim + 1];
...
char * opCodeTab[]
        = {"HALT","IN","OUT","ADD","SUB","MUL","DIV","INC","DEC","SHL","SHR","?RR",
            /* RR opcodes */
           "LD","ST","?RM", /* RM opcodes */
           "LDA","LDC","JLT","JLE","JGT","JGE","JEQ","JNE","?RA",
           /* RA opcodes */
           "?ALL"
          };
...
/********************************************/
int opClassCounterIndex( int c )
{ if      ( c <= opRRLim) return ( opRRLim );
  else if ( c <= opRMLim) return ( opRMLim );
  else                    return ( opRALim );
} /* opClassCounterIndex */
...
{ OPCODE op;
  ...
  { fgets( in_Line, LINESIZE-2, pgm  ) ;
    ...
    { if (! getNum())
      ...
      /* opcode counting */
      ++opCodeOverallCounter[op]; 
      ++opCodeOverallCounter[opLim]; 
      ++opCodeOverallCounter[opClassCounterIndex(op)]; 
      switch ( opClass(op) )
      { case opclRR :
        /***********************************/
...
{ INSTRUCTION currentinstruction  ;
  ...
  /* opcode counting */
  ++opCodeSimulationCounter[currentinstruction.iop]; 
  ++opCodeSimulationCounter[opLim]; 
  ++opCodeSimulationCounter[opClassCounterIndex(currentinstruction.iop)]; 
  ...
  { /* RR instructions */
    ...
    case opINC :  reg[r] = reg[r] + 1 ; break;

    case opDEC :  reg[r] = reg[r] - 1 ; break;

    case opSHL :  reg[r] = reg[r] << reg[t] ; break;

    case opSHR :  reg[r] = reg[r] >> reg[t] ; break;
  ...
  while (word[cmdi]) {
      ...
      { case 't' :
        ...
        /***********************************/
          ...
          for (i=0;i<opLim+1;++i) {
            if (i) printf(";");
            printf("%4s", opCodeTab[i]);
          }
          printf("\n");
          for (i=0;i<opLim+1;++i) {
            if (i) printf(";");
            printf("%4d", opCodeOverallCounter[i]);
          }
          printf("\n");
          for (i=0;i<opLim+1;++i) {
            if (i) printf(";");
            printf("%4d", opCodeSimulationCounter[i]);
          }
          printf("\n");
          break;
